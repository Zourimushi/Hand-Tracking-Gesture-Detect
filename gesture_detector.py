...
class gestureDetector:
    def __init__(self,
        ...
        base_options = python.BaseOptions(
            model_asset_path='Hand-Tracking-Gesture-Detect\hand_landmarker.task'
        )
        ...
        self.history_length = 10  # 保存最近10帧的位置
    ...
    def get_hand_center(self, detection_result, frame_shape):
        ...
        if detection_result.hand_landmarks:
            for hand_landmarks in detection_result.hand_landmarks:
                # 使用手腕(0)和食指根部(5)的中点作为手掌中心
                wrist = hand_landmarks[0]
                index_base = hand_landmarks[5]

                center_x = (wrist.x + index_base.x) / 2
                center_y = (wrist.y + index_base.y) / 2

                centers.append((int(center_x * w), int(center_y * h)))
    ...
    def detect_gesture(self, detection_result, frame_shape):
        ...
        centers = self.get_hand_center(detection_result, frame_shape)
        if not centers:
            return '无手势', 'none'
        ...
        self.hand_positions_history[hand_id].append(center)
        ...
        # 如果历史数据不足，无法判断手势
        if len(self.hand_positions_history[hand_id]) < self.history_length // 2:
            return '检测中...', 'none'

        # 获取位置历史
        history = list(self.hand_positions_history[hand_id])
        start_pos = history[0]
        end_pos = history[-1]
        ...
        # # 获取手的大小变化
        # size_history = list(self.hand_size_history[hand_id])
        # if len(size_history) >= self.history_length // 2:
        #     start_size = size_history[0]
        #     end_size = size_history[-1]
        #     size_change = end_size - start_size
        #
        #     # 如果水平垂直位移都很小，但手的大小变化明显
        #     if distance < self.move_threshold * 1.5:
        #         # 手变大 -> 前推（手靠近摄像头）
        #         if size_change > 15:  # 大小增加阈值
        #             return '向前推', 'forward'
        #         # 手变小 -> 后拉（手远离摄像头）
        #         elif size_change < -15:  # 大小减少阈值
        #             return '向后拉', 'backward'
    ...
    def run(self, camera_id=0, window_name='Hand Tracking',
        ...
        while cap.isOpened():
            ...
            if show_info:
                self.draw_finger_info(annotated_frame, detection_result,
                                      finger_counts, finger_statuses)

            # 检测并显示手势
            gesture_direction = self.draw_gesture_info(annotated_frame, detection_result, frame.shape)

            # 在手部位置绘制轨迹
            centers = self.get_hand_center(detection_result, frame.shape)
